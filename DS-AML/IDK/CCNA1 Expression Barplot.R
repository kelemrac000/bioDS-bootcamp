...
#lets just start with the first DEG in the DEGs dataframe called "CCNA1".
?filter
?gather
?inner_join
...
#Run the code below.
#Add comments that describe what each line of code is doing.
CCNA1.Expn <- Log2.Expn.df %>% #This line is defining a variable called "CCNA1.Expn" and piping the dataframe "Log2.Expn.df" into the filter() function.
  filter(., Gene %in% c("CCNA1")) %>% #This line is finding rows/columns where conditions are "TRUE". Only rows that are TRUE are kept while everything else is dropped. 
...
#9 ) Create a boxplot with the CCNA1 expression dataframe, CCNA1.Expn.
...
#uncomment this and complete the code
box1 <- ggplot(CCNA1.Expn, aes(x= group, y= Log2.Expression.Value, color= group)) +
  geom_boxplot() +
  theme_classic() +
  labs(title="CCNA1 expression")
...
#10 ) Create a waterfallplot with the CCNA1.Expn. expression dataframe.
#We want the column "group" to be the x-axis.
#and Make "Log2.Expression.Value" column the y-axis.
...
bar1 <- ggbarplot(CCNA1.Expn, x = "Sample", y = "Log2.Expression.Value", #set x and y axis to the column names "name" and "mpg".
...
bar2 <- ggbarplot(CCNA1.Expn, x = "Sample", y = "normalized.expression", #set x and y axis to the column names "name" and "mpg".
                  fill = "group",               # change fill color by column "cyl".
                  color = "group",            # Set bar border colors to white
                  sort.val = "desc",          # Sort the values in dscending order
                  sort.by.groups = TRUE,      # sort inside each group
